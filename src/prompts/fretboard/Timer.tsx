...
const showInfinityAt = 10_000;

export const Timer = ({ lastOkTime }: { lastOkTime: number }) => {
    ...
    const startCounter = useCallback(
        () => setInterval(() => setDelta(performance.now() - lastOkTime), 55),
        [lastOkTime]
    );
    ...
    }, [lastOkTime, startCounter]);

    useEffect(() => {
        if (delta < showInfinityAt) {
